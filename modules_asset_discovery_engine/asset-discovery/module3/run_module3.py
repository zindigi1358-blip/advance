...
def main():
    ...
    # BUG FIX: these must be set BEFORE any client module function runs —
    # nvd_client/vulners_client now read config.NVD_API_KEY / config.VULNERS_API_KEY
    # dynamically (via `import config`, not `from config import X`), so setting
    # them here correctly takes effect for the whole scan.
    if args.nvd_key:
