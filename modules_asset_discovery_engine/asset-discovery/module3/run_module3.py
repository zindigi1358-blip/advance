...
def process_subdomain(sub_record: dict, leak_findings: list, scan_id: str, domain: str) -> list:
    ...
    for tech in tech_iter:
        cve_list = []
        if tech["product"]:
            cve_list = nvd_client.query_nvd(tech["product"], tech["version"])
...
def notify_if_critical(summary: dict):
    """
    Placeholder hook — call your existing Discord webhook / email / Slack
    notifier here. Kept as a separate function so it's a one-line edit to
    wire into your existing alerting code from the honeypot/monitor project.

    Example:
        if summary["critical_count"] > 0 or summary["kev_matches"] > 0:
            send_discord_alert(summary)   # <- your existing function
    """
    if summary["kev_matches"] > 0:
        print(f"{RED}{BOLD}[ALERT HOOK] {summary['kev_matches']} actively-exploited "
              f"CVE(s) found — wire your notifier into notify_if_critical(){RESET}")
