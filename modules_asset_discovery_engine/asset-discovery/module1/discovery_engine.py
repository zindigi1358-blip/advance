...
class SubdomainDiscovery:
    ...
    async def from_threatintel(self) -> list[str]:
        """
        3 passive sources. ThreatCrowd dead. VT with apikey=0 removed (returned 204).
        Error logging now shows exception TYPE so empty errors are visible.
        """
        results = []

        def _err(source, e):
            # asyncio.TimeoutError / aiohttp errors have empty str() —
            # repr() always shows the actual exception class name
            logger.warning(f"[{source}] Error ({type(e).__name__}): {repr(e)}")

        # ── Source 1: AlienVault OTX ─────────────────────────────────────────
        # Key header: X-OTX-API-KEY  |  free at otx.alienvault.com/api
        try:
            url     = f"https://otx.alienvault.com/api/v1/indicators/domain/{self.domain}/passive_dns"
            headers = {"User-Agent": "Mozilla/5.0"}
            if self.otx_api_key:
                headers["X-OTX-API-KEY"] = self.otx_api_key
                logger.info("[AlienVault OTX] Using API key")
            async with aiohttp.ClientSession(
                ...
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        logger.warning("[AlienVault OTX] 403 — invalid API key or rate limit")
                    else:
                        logger.warning(f"[AlienVault OTX] HTTP {resp.status}")
        except Exception as e:
            _err("AlienVault OTX", e)

        # ── Source 2: URLScan.io ─────────────────────────────────────────────
        # Key header: API-Key  |  free at urlscan.io/user/profile/
        try:
            ...
            if self.urlscan_api_key:
                ...
                logger.info("[URLScan.io] Using API key")
            async with aiohttp.ClientSession(
                timeout=aiohttp.ClientTimeout(total=20), headers=headers
            ) as session:
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        for r in data.get("results", []):
                            pd = r.get("page", {}).get("domain", "")
                            if pd.endswith(f".{self.domain}") or pd == self.domain:
                                results.append(pd)
                        logger.info(f"[URLScan.io] Found {len(results) - before} entries")
                    elif resp.status == 429:
                        logger.warning("[URLScan.io] Rate limited — add API key to increase limit")
                    elif resp.status == 401:
                        logger.warning("[URLScan.io] 401 — invalid API key")
        except Exception as e:
            _err("URLScan.io", e)
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(url) as resp:
                    if resp.status == 200:
                        text  = await resp.text()
                        found = re.findall(
                            r'<td>([a-zA-Z0-9._-]+\.' + re.escape(self.domain) + r')</td>', text
                        )
    ...
    async def dns_bruteforce(self) -> list[str]:
        """
        DNS bruteforce — semaphore added.
        BUG FIX: no limit before meant all tasks fired at once, overwhelming
        DNS resolvers and causing false negatives.
        """
        results   = []
...
class CloudAssetDiscovery:
    ...
    async def check_azure_blob(self, session: aiohttp.ClientSession, name: str) -> Optional[CloudAsset]:
        """Check Azure Blob Storage — BUG FIX: was HEAD (no body), now GET."""
        ...
        try:
            async with session.get(url) as resp:
                ...
                if resp.status in [200, 400, 403]:
                    ...
                    return CloudAsset(asset_type="azure_blob", url=url,
                                      is_public=is_public, bucket_name=clean,
                                      provider="Azure",
                                      risk_level="critical" if is_public else "low")
                if resp.status == 404 and "StorageErrorResponse" in body:
                    return CloudAsset(asset_type="azure_blob", url=url,
                                      is_public=False, bucket_name=clean,
                                      provider="Azure", risk_level="low")
        except Exception:
            pass
        return None
                ...
                try:
                    ...
                    async with session.get(url, params=params) as resp:
                        if resp.status == 200:
                            ...
                            logger.warning(f"[GitHub] Rate limit — pausing {wait}s")
                            await asyncio.sleep(wait)

                    await asyncio.sleep(2)
                except Exception as e:
                    logger.warning(f"[GitHub] Error ({type(e).__name__}): {repr(e)}")
...
class AssetDiscoveryEngine:
    ...
    def __init__(
        ...
        # BUG FIX: lstrip("https://") strips chars NOT the string prefix.
        # "smtp.example.com" → lstrip removes leading 's' → "mtp.example.com"
        domain = domain.strip()
        if "://" in domain:
            _p = urlparse(domain)
