...
class SubdomainDiscovery:
    ...
    async def from_threatintel(self) -> list[str]:
        """Query AlienVault OTX + VirusTotal for subdomains (ThreatCrowd is dead)."""
        results = []

        # ── Source 1: AlienVault OTX (free, no key needed)
        try:
                    ...
                    if resp.status == 200:
                        ...
                        for entry in data.get("passive_dns", []):
                            ...
                            if host.endswith(f".{self.domain}") or host == self.domain:
                                results.append(host)
                        logger.info(f"[AlienVault OTX] Found {len(results)} entries")
        except Exception as e:
            logger.warning(f"[AlienVault OTX] Error: {e}")

        # ── Source 2: URLScan.io (completely free, no API key needed)
        # VT v2 with apikey=0 was removed — it returned 204 No Content and
        # never worked. URLScan.io is a working free replacement.
        try:
                    ...
                    if resp.status == 200:
                        ...
                        for result in data.get("results", []):
                            ...
                            if page_domain.endswith(f".{self.domain}") or page_domain == self.domain:
                                ...
                                urlscan_count += 1
                        logger.info(f"[URLScan.io] Found {urlscan_count} entries")
        except Exception as e:
            logger.warning(f"[URLScan.io] Error: {e}")

        # ── Source 3: RapidDNS (free, no key needed)
        try:
    ...
    async def dns_bruteforce(self) -> list[str]:
        """DNS bruteforce using wordlist — concurrency-limited to avoid overwhelming resolvers."""
        results = []
        resolver = dns.asyncresolver.Resolver()
        resolver.lifetime = 3.0
        # Limit to 50 concurrent DNS queries — firing all at once causes timeouts
        # and false negatives on slow/rate-limited resolvers (was the real cause
        # of many "not found" results on large wordlists)
...
class CloudAssetDiscovery:
    ...
    async def check_azure_blob(self, session: aiohttp.ClientSession, name: str) -> Optional[CloudAsset]:
        ...
        BUG FIX: Was using HEAD which returns no body — 'StorageErrorResponse'
        check was always False, making the 404 branch dead code. Using GET now
        so Azure's XML error body is actually readable."""
        clean = name.replace("-", "")[:24]  # Azure storage names: lowercase alnum, max 24
        ...
        try:
            async with session.get(url) as resp:
                ...
                # 404 with StorageErrorResponse in body = account exists, container not found
                if resp.status == 404 and "StorageErrorResponse" in body_text:
                    return CloudAsset(
                        asset_type="azure_blob",
                        url=url,
                        is_public=False,
                ...
                try:
                    ...
                    async with session.get(url, params=params) as resp:
                        if resp.status == 200:
                            ...
                            # Read Retry-After if GitHub sends it, else wait 60s.
                            # 10s was never enough — GitHub's rate limit window is 60s
                            # for search and up to 3600s for secondary limits.
                            retry_after = int(resp.headers.get("Retry-After", 60))
                            logger.warning(f"[GitHub] Rate limit hit — pausing {retry_after}s")
...
class AssetDiscoveryEngine:
    ...
    def __init__(
        ...
        # Sanitize domain — lstrip("https://") is WRONG (strips individual chars,
        # not the prefix string). "staging.example.com" passed as-is would have
        # its leading 's' stripped by lstrip("https://") since 's' is in that
        # character set. Use urlparse instead.
        domain = domain.strip()
        if "://" in domain:
            _parsed = urlparse(domain)
            domain  = _parsed.netloc or _parsed.path
    ...
    async def run(self) -> DiscoveryReport:
        ...
        discovery = SubdomainDiscovery(self.domain)
